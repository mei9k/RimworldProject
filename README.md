# RimworldProject

Repositorio de documentación para un sistema de rol sci-fi futurista-realista basado en RapidoYFacil 3.0.

## Entrada rápida

- [Índice general](website-fuma/content/docs/indice.md)
- [Guía de desarrollo local](website-fuma/content/docs/desarrollo-local.md)
- [Material Sanitario](website-fuma/content/docs/Compendios/material-sanitario.mdx)
- [Resumen inicial del proyecto](mainOverview.md)
- [Sistema de juego](website-fuma/content/docs/01-sistema/concepto-y-estilo.md)
- [Universo y lore](website-fuma/content/docs/02-universo/resumen.md)
- [Partidas y plantillas](website-fuma/content/docs/03-partidas/indice.md)

## Estructura actual

La documentación viva del proyecto se mantiene solo en `website-fuma/content/docs/`.

La carpeta `website-fuma/` contiene tanto el frontend Fumadocs como el contenido fuente que publica la web.

